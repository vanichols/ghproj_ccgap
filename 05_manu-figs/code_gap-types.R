...
show_col(colours = c(xdb, xlb, xgr, xyel, "red", xpur))

show_col(c("red", "dodgerblue", "purple4", cc_color, xdp))
...
dat %>%
  ...
  geom_text(x = 18, y = 2700, label = "36 site-years, undetermined",
            hjust = 0, check_overlap = T, fontface = "italic", color = "gray50") +
  geom_text(x = 40, y = 2700, label = "6 site-years, no yield gap",
            hjust = 0, check_overlap = T, fontface = "italic", color = "gray50") +
  geom_text(x = 46, y = 2700, label = "6 site-years, yield gap from N factors",
            hjust = 0, check_overlap = T, fontface = "italic", color = "gray50") +
  geom_text(x = 90, y = 2700, label = "109 site-years, yield gap from\n  mixture of factors",
            hjust = 0, check_overlap = T, fontface = "italic", color = "gray50") +
  scale_fill_manual(values = c("red", "purple4")) +
...
dat %>%
  ...
  mutate(id = 1:n(),
         nonngap = ifelse(nonngap == 0 & ngap == 0, 10, nonngap),
         ngap = ifelse(is.na(nonngap), 10, ngap),
  ...
  geom_text(x = -5, y = 2700, label = "36 site-years, undetermined", 
            hjust = 0, check_overlap = T, fontface = "italic") +
  geom_text(x = 4, y = 2700, label = "6 site-years, no yield gap", 
            hjust = 0, check_overlap = T, fontface = "italic") +
  geom_text(x = 10, y = 2700, label = "6 site-years, yield gap from N factors", 
            hjust = 0, check_overlap = T, fontface = "italic") +
  geom_text(x = 60, y = 2700, label = "109 site-years, yield gap from\n  mixture of N- and non-N mechanisms", 
            hjust = 0, check_overlap = T, fontface = "italic") +
  scale_fill_manual(values = c("red", "purple4")) +
  scale_y_continuous(limits = c(0, 6000)) +
  ...
  theme(legend.position = "top") +
  labs(fill = NULL,
       y = "Yield gap between continuous- and rotated-maize (kg ha-1)",
       x = "Site-year") +
  coord_flip()
