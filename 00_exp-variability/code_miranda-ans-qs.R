...
# updated: 2/5/2021, miranda's stuff (2nd round)
...
# note: this is assuming we can't pool across years within a site. 
# If we believe the variance is the same for a given site, we could do that. It would make the CIs tighter. 
# But:

#--variation by year
dat %>% 
  select(-cc, -sc, -gap_kgha) %>% 
  pivot_longer(cc_sd:sc_sd) %>% 
  ggplot(aes(site, value, color = name)) + 
  geom_jitter() + 
...
#--just pool within a site-year
dat2 <- 
  dat %>% 
  mutate(cc_s2 = (cc_sd) ^2,
         sc_s2 = (sc_sd) ^2,
         sp = sqrt(3 * (cc_s2 + sc_s2)/(4 + 4 - 2)),
         gap_se = sp * sqrt(1/4 + 1/4))

#--write it to use in other places
dat2 %>% 
  select(site, year, gap_kgha, gap_se, gap_hi, gap_lo, sig_ind) %>% 
...
dat_sig %>% 
  mutate(site = as.factor(site),
         year = reorder_within(year, -gap_kgha, site)) %>% 
  ggplot(aes(year, gap_kgha)) + 
  geom_linerange(aes(ymin = gap_kgha - 1.96*gap_se,
                     ymax = gap_kgha + 1.96*gap_se,
                     color = sig_ind), size = 1.5) +
  geom_hline(yintercept = 0, linetype = "dashed") +
