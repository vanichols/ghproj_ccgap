...
#--sim data
ames <- read_csv("02_ames.csv") %>% mutate(site = "ames")
nash <- read_csv("02_nash.csv") %>% mutate(site = "nash")
...
sims %>% 
  ...
  geom_line(aes(color = rot2, group = interaction(rot2, set_desc),
                linetype = till), size = 2) +
  facet_grid(name~till+site) +
  scale_color_manual(values = c("cc" = "orange2", "sc" = "green4")) +
  labs(title = "Apsim w/o scripts (top) and observed (bottom)",
...
o.gaps %>% 
  ...
  select(set_desc, site, year, obs_gap_kgha, sim_gap_kgha) %>%
  filter(!is.na(obs_gap_kgha), !is.na(sim_gap_kgha), set_desc == "1-no scripts") %>% 
  pivot_longer(obs_gap_kgha:sim_gap_kgha) %>% 
  group_by(site, set_desc, name) %>% 
  arrange(value) %>% 
  mutate(n = 1:n()) %>% 
  ggplot(aes(n, value)) + 
  geom_point(aes(color = name)) +
  facet_wrap(site~set_desc, scales = "free", labeller = label_wrap_gen())
