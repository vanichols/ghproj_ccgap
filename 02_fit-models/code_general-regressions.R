...
imps <- read_csv("02_fit-models/fm-rfpct-imp-raw-loo.csv")
...
dat %>% 
  pivot_longer(prev_ccyield:iacsr) %>% 
  filter(name %in% c("heatstress_n", "prep2wk_precip_mm_tot")) %>% 
  ggplot(aes(value, cgap_max)) + 
  geom_point(aes(color = site)) + 
  geom_smooth(method = "lm") + 
  facet_wrap(.~name + site, scales = "free")


#--maybe an interaction btwn heatstress and precip?
Heatstres seems to depend more heavily on the site
...
mypreds <- predict(myRR_mod, s = bestlam, newx = myx_tmp) %>% 
  as_tibble() %>% 
  pull()
...
#--how do the predictions look?
mypreds <- predict(myL_mod, s = bestlam, newx = myx_tmp) %>% 
  as_tibble() %>% 
  pull()
...
datraw %>%
  select(heatstress_n, cgap_max, cgap_max) %>% 
