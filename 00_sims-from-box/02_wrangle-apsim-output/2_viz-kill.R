...
#--sim data
ames <- 
  read_csv("02_ames.csv") %>% 
  filter(grepl("kill|scripts", set_desc)) %>% 
  mutate(set_desc = paste(set_id, set_desc2, sep = "-"))

nash <- 
  read_csv("02_nash.csv") %>% 
  filter(grepl("kill|scripts", set_desc)) %>% 
  mutate(set_desc = paste(set_id, set_desc2, sep = "-"))
...
#--is the kill script affecting the gap, compared to no scripts?
s.gaps %>% 
  #  filter(set_id == 2) %>% 
  left_join(o.gaps) %>% 
  select(site, set_id, set_desc, till, year, sim_gap_buac, obs_gap_buac) %>% 
  mutate(set_cat = case_when(
    grepl("only dry", set_desc) ~ "dry pen",
    grepl("no dry|only wet", set_desc) ~ "wet pen",
    grepl("only cold", set_desc) ~ "temp pen",
    TRUE ~ "other")) %>% 
  pivot_longer(sim_gap_buac:obs_gap_buac) %>% 
  pivot_wider(names_from = name, values_from = value) %>% 
  ggplot() +
  geom_line(aes(year, sim_gap_buac, color = set_desc), size = 2) + 
  geom_line(aes(year, obs_gap_buac), linetype = "dashed") + 
  facet_grid(site~till+set_cat)+
  scale_color_manual(values = col_vector) +
  labs(title = "Kill script generally inc gap size",
       subtitle = "But 10% wet kill needed to match obs gap")
...
sims %>% 
  ...
  facet_grid(till+site~set_desc, scales = "free") + 
  labs(title = "More plants are killed in NT CC compared to SC")

#--umm nash has 100% kill in one of the years
#--what happened in 2009?
sims %>% 
  mutate(pct_kill = (PlantDensityS - PlantDensityH)/PlantDensityS * 100) %>% 
  filter(pct_kill > 70) %>% 
  filter(set_id == 1) %>% 
  select(-Date, -CropYield)

sims %>% 
  filter(set_id == 1, site == "nash", till == "normal") %>% 
  select(site, set_desc, till, rot2, year, PlantDensityS, PlantDensityH) %>% 
  mutate(pct_kill = (PlantDensityS - PlantDensityH)/PlantDensityS * 100) %>% 
  ggplot(aes(year, pct_kill, group = interaction(set_desc, rot2))) + 
  geom_line(aes(color = interaction(rot2, site)), size = 2) + 
  facet_grid(till+site~set_desc, scales = "free") + 
  labs(title = "No scripts, no plants should die",
       subtitle = "What happend in 2009? We still get yield.")
