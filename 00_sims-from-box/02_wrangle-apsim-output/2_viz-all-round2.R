...
obs1 <- 
  ...
  select(-crop, -nrate_kgha, -rotation) %>% 
...
obs1 %>% 
  ggplot(aes(env_ind, ogap_kgha)) + 
  geom_point(aes(pch = state, color = state), size = 2)
  

obs1 %>% 
  group_by(site) %>% 
  mutate(site_ind = mean(env_ind, na.rm = T)) %>% 
  ggplot(aes(reorder(site, site_ind), ogap_kgha)) + 
  geom_point(aes(size = site_ind)) +
  geom_hline(yintercept = 0) +
  labs(title = "Size of penalty not related to average site productivity")
